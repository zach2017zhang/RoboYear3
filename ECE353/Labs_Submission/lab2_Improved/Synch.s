...
_StringConst_4:
	.word	48			! length
	.ascii	"Current thread already holds a lock on the mutex"
	.align
_StringConst_3:
	.word	51			! length
...
!   Send message IsHeldByCurrentThread
	...
	add	r2,16,r2
	call	r2
!   Retrieve Result: targetName=_temp_64  sizeInBytes=1
	loadb	[r15],r1
	storeb	r1,[r14+-10]
!   _temp_66 = _temp_64 XOR 1		(bool)
	loadb	[r14+-10],r1
	mov	1,r2
	xor	r1,r2,r1
	storeb	r1,[r14+-9]
!   if _temp_66 then goto _Label_62 else goto _Label_63
	loadb	[r14+-9],r1
	cmp	r1,0
	be	_Label_63
	jmp	_Label_62
...
! IF STATEMENT...
	...
	mov	"\0\0IF",r10
!   if intIsZero (heldBy) then goto _Label_67
	load	[r14+8],r1
	load	[r1+4],r1
	cmp	r1,r0
	be	_Label_67
...
! IF STATEMENT...
	...
	load	[r14+8],r1
	store	r1,[r14+-32]
!   if intIsZero (_temp_85) then goto _runtimeErrorNullPointer
	load	[r14+-32],r1
	cmp	r1,r0
	be	_runtimeErrorNullPointer
!   Send message IsHeldByCurrentThread
	...
	add	r2,16,r2
	call	r2
!   Retrieve Result: targetName=_temp_84  sizeInBytes=1
	loadb	[r15],r1
	storeb	r1,[r14+-10]
!   _temp_86 = _temp_84 XOR 1		(bool)
	loadb	[r14+-10],r1
	mov	1,r2
	xor	r1,r2,r1
	storeb	r1,[r14+-9]
!   if _temp_86 then goto _Label_82 else goto _Label_83
	loadb	[r14+-9],r1
	cmp	r1,0
	be	_Label_83
	jmp	_Label_82
...
! IF STATEMENT...
	...
	mov	"\0\0IF",r10
!   if intIsZero (t) then goto _Label_91
	load	[r14+-36],r1
	cmp	r1,r0
	be	_Label_91
!	jmp	_Label_90
...
! IF STATEMENT...
	...
	mov	"\0\0IF",r10
!   if _P_Thread_currentThread != heldBy then goto _Label_106		(int)
	set	_P_Thread_currentThread,r1
	load	[r1],r1
	load	[r14+8],r2
	load	[r2+4],r2
...
! IF STATEMENT...
	...
	mov	"\0\0IF",r10
!   if t == 0 then goto _Label_133		(int)
	load	[r14+-36],r1
	mov	0,r2
	cmp	r1,r2
	be	_Label_133
...
!   _temp_134 = t + 76
	...
	add	r1,76,r1
	store	r1,[r14+-20]
!   Data Move: *_temp_134 = 2  (sizeInBytes=4)
	mov	2,r1
	load	[r14+-20],r2
	store	r1,[r2]
! SEND STATEMENT...
...
!   Retrieve Result: targetName=_temp_148  sizeInBytes=1
	loadb	[r15],r1
	storeb	r1,[r14+-9]
!   if _temp_148 then goto _Label_147 else goto _Label_146
	loadb	[r14+-9],r1
	cmp	r1,0
	be	_Label_146
	jmp	_Label_147
...
! IF STATEMENT...
	...
	mov	"\0\0IF",r10
!   if intIsZero (t) then goto _Label_154
	load	[r14+-36],r1
	cmp	r1,r0
	be	_Label_154
	jmp	_Label_155
...
!   _temp_156 = t + 76
	...
	add	r1,76,r1
	store	r1,[r14+-20]
!   Data Move: *_temp_156 = 2  (sizeInBytes=4)
	mov	2,r1
	load	[r14+-20],r2
	store	r1,[r2]
! SEND STATEMENT...
