  ...
  behavior Mutex
	...
	endMethod
	  ...
	  if self.IsHeldByCurrentThread() == true
	    FatalError ("Current thread already holds a lock on the mutex")
	  else
	    if heldBy == null
	      ...
	      currentThread.Sleep ()
              -- print (currentThread.name)
              -- print ("has been added to waiting list and slept\n")
	    endIf
	  endIf
	  oldIntStat = SetInterruptsTo (oldIntStat)
	endMethod
	  ...
	  if self.IsHeldByCurrentThread() != true
	    ...
	    if t != null
		  ...
		  heldBy = t
              -- print (t.name)
              -- print ("has been added to ready list")
	    endIf
	  endIf
	  oldIntStat = SetInterruptsTo (oldIntStat)
	endMethod
